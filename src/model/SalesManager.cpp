...
const Flow SalesManager::calc_production_X() {
    ...
    std::sort(
            ...
            [](const std::shared_ptr<BusinessConnection>& business_connection_1,
               const std::shared_ptr<BusinessConnection>& business_connection_2) {
                if (business_connection_1->last_demand_request_D().get_quantity() <= 0.0 &&
                    business_connection_2->last_demand_request_D().get_quantity() > 0.0) {
                ...
                if (business_connection_1->last_demand_request_D().get_price() >
                    business_connection_2->last_demand_request_D().get_price()) {
                    return true;
                }
                if (business_connection_1->last_demand_request_D().get_price() <
                    business_connection_2->last_demand_request_D().get_price()) {
                ...
                return (business_connection_1->last_demand_request_D().get_quantity() >
                        business_connection_2->last_demand_request_D().get_quantity());
            });
    ...
    std::tie(communicated_parameters_.production_X,
             communicated_parameters_.offer_price_n_bar) = calc_supply_distribution_scenario(possible_production_X_hat);
...
void SalesManager::distribute(const Flow& _) {
    ...
    if (communicated_parameters_.production_X.get_quantity() <= 0.0) {  // no production
        for (auto not_served_bc = business_connections.begin();
             not_served_bc != business_connections.end(); ++not_served_bc) {
        ...
        for (auto served_bc = business_connections.begin(); served_bc != business_connections.end(); ++served_bc) {
            if ((*served_bc)->last_demand_request_D().get_quantity() > 0.0) {
                if (round((*served_bc)->last_demand_request_D().get_price() -
                    ...
                    assert((*served_bc)->last_demand_request_D().get_quantity() <=
                           communicated_parameters_.production_X.get_quantity());
        ...
        if (begin_cheapest_price_range < end_cheapest_price_range) {
            const FlowQuantity production_cheapest_price_range =
                    round(communicated_parameters_.production_X.get_quantity() -
                          production_without_cheapest_price_range.get_quantity());
            if (demand_cheapest_price_range >
                production_cheapest_price_range) {  // not all demands in cheapest price range can be fulfilled
                ...
                const Price average_price_cheapest_price_range =
                        demand_value_cheapest_price_range / demand_cheapest_price_range;
                const Price price_shift = std::max(
                        (max_price_cheapest_price_range * production_cheapest_price_range -
                         average_price_cheapest_price_range * demand_cheapest_price_range)
                ...
                const Price price_as_calculated_in_distribution_scenario =
                        (communicated_parameters_.production_X.get_value() -
                         production_without_cheapest_price_range.get_value()) / production_cheapest_price_range;
                ...
                for (auto served_bc = begin_cheapest_price_range; served_bc != end_cheapest_price_range; ++served_bc) {
                    Flow flow_Z = Flow(round(FlowQuantity(to_float(production_cheapest_price_range)
                                                          * to_float(
                            (*served_bc)->last_demand_request_D().get_quantity() * price_shift
                            + (*served_bc)->last_demand_request_D().get_value())
                                                          / to_float(
                            demand_cheapest_price_range * price_shift + demand_value_cheapest_price_range))),
                                       model()->parameters().cheapest_price_range_preserve_seller_price
                                       ? price_as_calculated_in_distribution_scenario
...
std::tuple<Flow, Price> SalesManager::calc_supply_distribution_scenario(const Flow& possible_production_X_hat_p) {
    ...
    auto first_zero_connection = std::find_if(business_connections.begin(), business_connections.end(),
                                              [](const std::shared_ptr<BusinessConnection>& business_connection) {
                                                  return business_connection->last_demand_request_D().get_quantity() <=
                                                         0.0;
                                              });
    ...
    if (first_zero_connection == business_connections.begin()  // no production due to demand quantity shortage
        || round((*business_connections.begin())->last_demand_request_D().get_price()) <
           minimal_production_price  // no production due to demand value shortage
            ) {
    ...
    for (auto business_connection = business_connections.begin();
         business_connection != first_zero_connection; ++business_connection) {
        // check that connection can be served at all
        if (round((production_X + (*business_connection)->last_demand_request_D())).get_quantity() >
            possible_production_X_hat_p.get_quantity()) {
        ...
        Price maximal_marginal_production_costs =
                calc_marginal_production_costs(
                        (round(production_X + (*business_connection)->last_demand_request_D())).get_quantity(),
                        minimal_production_price);
        if (isnan(maximal_marginal_production_costs)  // infinite costs
            || round((*business_connection)->last_demand_request_D().get_price()) <
               round(maximal_marginal_production_costs)) {
            supply_distribution_scenario.connection_not_served_completely = business_connection;
            break;
        }
    ...
    if (supply_distribution_scenario.connection_not_served_completely == first_zero_connection) {
        ...
        auto cheapest_served_non_zero_connection =
                first_zero_connection - 1;  // note: first_zero_connection == business_connections.begin() is excluded
        assert(production_X.get_quantity() <=
               sum_demand_requests_D_.get_quantity());  // to see if we make rounding errors
        assert(production_X.get_quantity() >=
               sum_demand_requests_D_.get_quantity());  // to see if we make rounding errors
            ...
            const Price offer_price_n_bar = round(std::max(
                    ...
                    * (1
                       + firm->sector->parameters().supply_elasticity
                         * ((production_X - (firm->forced_initial_production_lambda_X_star())) /
                            (firm->forced_initial_production_lambda_X_star()))),
                    minimal_offer_price));
            supply_distribution_scenario.price_cheapest_buyer_accepted_in_optimization =
    ...
    if (round(goal_fkt_marginal_costs_minus_price(production_X.get_quantity(), minimal_production_price,
        ...
        if (round(goal_fkt_marginal_costs_minus_price(possible_production_X_hat_p.get_quantity(),
                                                      minimal_production_price,
                                                      (*supply_distribution_scenario.connection_not_served_completely)->last_demand_request_D().get_price()))
            < 0.0)
...
std::tuple<Flow, Price>
SalesManager::calc_expected_supply_distribution_scenario(const Flow& possible_production_X_hat_p) {
    ...
    if (first_zero_connection == business_connections.begin()  // no production due to demand quantity shortage
        ...
        Flow expected_production_X =
                Flow(std::min(firm->forced_initial_production_lambda_X_star().get_quantity(),
                              possible_production_X_hat_p.get_quantity()), offer_price_n_bar);
    ...
    for (auto business_connection = business_connections.begin();
        ...
        if (round((expected_production_X + (*business_connection)->last_demand_request_D())).get_quantity() >
            possible_production_X_hat_p.get_quantity()) {
            supply_distribution_scenario.connection_not_served_completely = business_connection;
            break;
        }
        ...
        if (!model()->parameters().respect_markup_in_production_extension                                       // we always want to attract more demand
            || expected_production_X.get_quantity() <
               firm->forced_initial_production_quantity_lambda_X_star()  // ... production is below lambda * X_star
                ) {
            maximal_marginal_production_costs = calc_marginal_production_costs(
                    (round(expected_production_X + (*business_connection)->last_demand_request_D())).get_quantity(),
                    minimal_production_price);
        } else {
            maximal_marginal_production_costs = calc_marginal_production_costs(
                    (round(expected_production_X + (*business_connection)->last_demand_request_D())).get_quantity(),
    ...
    if (supply_distribution_scenario.connection_not_served_completely == first_zero_connection) {
        ...
        if ((model()->parameters().always_extend_expected_demand_curve                                           // we always want to attract more demand
            ...
            if (round(goal_fkt_marginal_costs_minus_marginal_revenue(
                ...
                FlowQuantity maximal_additional_production_quantity =
                        round(std::min(possible_production_X_hat_p.get_quantity(),
                                       firm->forced_maximal_production_quantity_lambda_beta_X_star())
                              - expected_production_X.get_quantity());
                if (round(goal_fkt_marginal_costs_minus_marginal_revenue(
                    ...
                    additional_expected_flow =
                            Flow(maximal_additional_production_quantity,
                                 calc_additional_revenue_expectation(round(expected_production_X.get_quantity() +
                                                                           maximal_additional_production_quantity),
                                                                     (*cheapest_served_non_zero_connection)->last_demand_request_D().get_price()));
                } else {
                    // price is not high enough to fulfill complete demand request, we have to find the optimal production ratio
                    Price precision_root_finding = Price(Price::precision);
                    additional_expected_flow =
                            search_root_bisec_expectation(FlowQuantity(0.0), maximal_additional_production_quantity,
                                                          expected_production_X.get_quantity(),
                                                          respect_markup_in_extension_of_revenue_curve
                                                          ? minimal_offer_price : minimal_production_price,
                                                          (*cheapest_served_non_zero_connection)->last_demand_request_D().get_price(),
        ...
        const Price offer_price_n_bar = round(std::max(
                expected_production_X.get_price(),
                calc_total_production_costs(expected_production_X, minimal_offer_price) /
                expected_production_X.get_quantity()));
...
void SalesManager::iterate_expectation() {
    ...
    if (estimated_possible_production_X_hat_.get_quantity() > 0.0) {
        estimated_possible_production_X_hat_.set_price(estimated_possible_production_X_hat_.get_price()
                                                       / (1 -
                                                          tax_));  // tax is applied here (1-tax_) because tax is on revenue!
    }
...
void SalesManager::initialize() {
    ...
    for (auto input_storage = firm->input_storages.begin();
         input_storage != firm->input_storages.end(); ++input_storage) {
    ...
    estimated_possible_production_X_hat_.set_price(
            initial_unit_commodity_costs + get_initial_unit_variable_production_costs());
}

const FlowValue SalesManager::calc_total_production_costs(const Flow& production_X,
    ...
    if (production_quantity_X <=
        firm->forced_initial_production_quantity_lambda_X_star()) {  // not in production extension
        return production_quantity_X * unit_production_costs_n_c;
    }
    // in production extension
    return production_quantity_X * unit_production_costs_n_c +
           calc_production_extension_penalty_P(production_quantity_X);
}

const FlowValue
SalesManager::calc_production_extension_penalty_P(const FlowQuantity& production_quantity_X) const {
    assert(production_quantity_X >= 0.0);
    if (production_quantity_X <=
        firm->forced_initial_production_quantity_lambda_X_star()) {  // not in production extension
    ...
    return firm->sector->parameters().price_increase_production_extension /
           (2.0 * firm->forced_initial_production_quantity_lambda_X_star())
...
const Price
SalesManager::calc_marginal_production_extension_penalty(const FlowQuantity& production_quantity_X) const {
    assert(production_quantity_X >= 0.0);
    if (production_quantity_X <=
        firm->forced_initial_production_quantity_lambda_X_star()) {  // not in production extension
        return Price(0.0);
...
const Price SalesManager::calc_marginal_production_costs(const FlowQuantity& production_quantity_X,
                                                         const Price& unit_production_costs_n_c) const {
    assert(production_quantity_X >= 0.0);
    assert(!isnan(unit_production_costs_n_c));
    if (production_quantity_X <=
...
const FlowQuantity SalesManager::analytic_solution_in_production_extension(const Price& unit_production_costs_n_c,
    ...
    return round(
            firm->forced_initial_production_quantity_lambda_X_star()
            * (1.0 + (price_demand_request_not_served_completely - unit_production_costs_n_c) /
                     firm->sector->parameters().price_increase_production_extension));
}

const FlowValue SalesManager::calc_additional_revenue_expectation(const FlowQuantity& production_quantity_X_p,
    ...
    return n_min_p
           * (production_quantity_X_p * pow(sum_demand_requests_D_.get_quantity() / production_quantity_X_p,
                                            firm->sector->parameters().supply_elasticity)
              - sum_demand_requests_D_.get_quantity())
           / (1.0 - firm->sector->parameters().supply_elasticity);
}

const Price SalesManager::calc_marginal_revenue_curve(const FlowQuantity& production_quantity_X_p,
                                                      const Price& n_min_p) const {
    // note: for X > sum_of_demand_requests_D this curve corresponds to the marginal supply curve
    // note: communicated_parameters_.production_X == sum_demand_requests_D (up to rounding issues)
    assert(round(production_quantity_X_p) >= sum_demand_requests_D_.get_quantity());
    ...
    return n_min_p * pow(sum_demand_requests_D_.get_quantity() / production_quantity_X_p,
                         firm->sector->parameters().supply_elasticity);
}

const Price
SalesManager::goal_fkt_marginal_costs_minus_marginal_revenue(const FlowQuantity& production_quantity_X_p,
...
const Flow SalesManager::search_root_bisec_expectation(const FlowQuantity& left,
    ...
    if (left + FlowQuantity(FlowQuantity::precision) >= right) {  // interval too narrow
        if (abs(goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + left,
                                                               unit_production_costs_n_c, n_min_p))
            < abs(goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + right,
                                                                 unit_production_costs_n_c, n_min_p))) {
    ...
    // check if root is in interval
    assert(!same_sgn(
            goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + left, unit_production_costs_n_c,
                                                           n_min_p),
            goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + right, unit_production_costs_n_c,
                                                           n_min_p)));

    if (abs(goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + middle, unit_production_costs_n_c,
                                                           n_min_p)) < precision_p) {
        return Flow(middle, calc_additional_revenue_expectation(production_quantity_X_p + middle, n_min_p));
    }
    if (abs(goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + left, unit_production_costs_n_c,
                                                           n_min_p)) < precision_p) {
        return Flow(left, calc_additional_revenue_expectation(production_quantity_X_p + left, n_min_p));
    }
    if (abs(goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + right, unit_production_costs_n_c,
                                                           n_min_p)) < precision_p) {
        return Flow(right, calc_additional_revenue_expectation(production_quantity_X_p + right, n_min_p));
    }
    if (!same_sgn(
            goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + middle, unit_production_costs_n_c,
                                                           n_min_p),
            goal_fkt_marginal_costs_minus_marginal_revenue(production_quantity_X_p + right, unit_production_costs_n_c,
                                                           n_min_p))) {
        return search_root_bisec_expectation(middle, right, production_quantity_X_p, unit_production_costs_n_c, n_min_p,
                                             precision_p);
    }
    return search_root_bisec_expectation(left, middle, production_quantity_X_p, unit_production_costs_n_c, n_min_p,
                                         precision_p);
}
...
void SalesManager::print_parameters() const {
    info(PRINT_ROW1("X", communicated_parameters_.production_X.get_quantity())
                 ...
                 << PRINT_ROW1("X_hat", communicated_parameters_.possible_production_X_hat)
                 << PRINT_ROW1("  lambda", firm->forcing())
                 ...
                 << PRINT_ROW1("p", communicated_parameters_.production_X.get_quantity() /
                                    firm->initial_production_X_star().get_quantity())
                 << PRINT_ROW1("p_exp", communicated_parameters_.expected_production_X.get_quantity() /
                                        firm->initial_production_X_star().get_quantity())
                 << PRINT_ROW1("m_prod_cost(X)",
                               calc_marginal_production_costs(communicated_parameters_.production_X.get_quantity(),
                                                              communicated_parameters_.possible_production_X_hat.get_price() *
                                                              (1 - tax_)))
                 << PRINT_ROW1("expected m_prod_cost(X)", calc_marginal_production_costs(
                         communicated_parameters_.expected_production_X.get_quantity(),
                 ...
                 << PRINT_ROW1("n_c", communicated_parameters_.possible_production_X_hat.get_price())
...
void SalesManager::print_connections(
...
#pragma omp critical(output)
    {
        std::cout << model()->run()->timeinfo() << ", " << id() << ": supply distribution for "
                  << business_connections.size() << " outputs :\n";
        FlowQuantity sum = FlowQuantity(0.0);
        FlowQuantity initial_sum = FlowQuantity(0.0);
        for (const auto& bc : business_connections) {
            std::cout << "      " << bc->id() << " :\n"
                      << PRINT_ROW1("n", bc->last_demand_request_D().get_price())
                      << PRINT_ROW1("D_r", bc->last_demand_request_D().get_quantity())
                      << PRINT_ROW1("D_star", bc->initial_flow_Z_star().get_quantity())
                      << PRINT_ROW1("Z_last", bc->last_shipment_Z(this).get_quantity())
                      << PRINT_ROW1("in_share",
                                    (bc->initial_flow_Z_star() / bc->buyer->storage->initial_input_flow_I_star()))
                      << '\n';
        ...
        if (supply_distribution_scenario.connection_not_served_completely != business_connections.end()) {
            std::cout << "      not completely served: "
                      << (*supply_distribution_scenario.connection_not_served_completely)->id() << '\n';
        } else {
...
const FlowValue SalesManager::get_tax() const {
