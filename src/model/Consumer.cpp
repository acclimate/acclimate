...
void Consumer::initialize() {
    ...
    if (utilitarian) {
        ...
        for (auto& input_storage : input_storages) {
            Flow initial_consumption = input_storage->initial_used_flow_U_star();
...
FloatType Consumer::CES_marginal_utility(int index_of_good, FlowQuantity consumption_demand) const {
    return pow(to_float(consumption_demand), (substitution_exponent - 1)) * substitution_exponent * share_factors[index_of_good];
}
...
void Consumer::iterate_consumption_and_production() {
    ...
    if (utilitarian) {
        ...
        for (const auto& is : input_storages) {
            Flow possible_consumption_flow = is->get_possible_use_U_hat();
            possible_consumption.push_back((possible_consumption_flow.get_quantity()));
            consumption_prices.push_back(possible_consumption_flow.get_price());
        }
        ...
        auto used_budget = FlowValue(0.0);
        for (std::size_t r = 0; r < possible_consumption.size(); ++r) {
            const FlowQuantity& desired_consumption_quantity =
                FlowQuantity(std::min(to_float(previous_consumption[r]), to_float(possible_consumption[r])));  // cap desired consumption at maximum possible
            desired_consumption.push_back(Flow(desired_consumption_quantity, consumption_prices[r]));
            used_budget += desired_consumption[r].get_value();
        }

        // adjust if price changes make previous consumption to expensive - assuming constant expenditure per good
        if (to_float(budget + not_spent_budget - used_budget) != 0) {
            ...
            for (std::size_t r = 0; r < desired_consumption.size(); ++r) {
                desired_consumption[r] = Flow(
                    std::min(possible_consumption[r], desired_consumption[r].get_quantity() * to_float(previous_prices[r]) / to_float(consumption_prices[r])),
        ...
        for (std::size_t r = 0; r < desired_consumption.size(); ++r) {
            optimizer_consumption.push_back(to_float((desired_consumption[r].get_quantity())));
...
#pragma omp critical(output)
        {
            ...
            for (std::size_t r = 0; r < consumption_demands.size(); ++r) {
                ...
                print_row("consumption request", desired_consumption[r].get_quantity());
                print_row("consumption price", consumption_prices[r]);
                print_row("current utility", utility);
                print_row("current total budget", budget);
                print_row("current spending for this good", desired_consumption[r].get_value());
