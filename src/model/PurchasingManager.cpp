...
PurchasingManager::~PurchasingManager() {
    ...
    //TODO: What is the correct destructor? In PurchasingManagerPrices it was set to default
}
...
FloatType PurchasingManager::estimate_production_extension_penalty(const BusinessConnection* bc,
...
FloatType PurchasingManager::estimate_marginal_production_extension_penalty(const BusinessConnection* bc,
                                                                            FloatType production_quantity_X) const {
    assert(production_quantity_X >= 0.0);
    if (production_quantity_X <=
        bc->seller->firm->forced_initial_production_quantity_lambda_X_star_float()) {  // not in production extension
...
inline FloatType PurchasingManager::scaled_D_r(FloatType D_r, const BusinessConnection* bc) const {
    return D_r / partial_D_r_scaled_D_r(bc);
}

inline FloatType PurchasingManager::unscaled_D_r(FloatType x, const BusinessConnection* bc) const {
    return x * partial_D_r_scaled_D_r(bc);
}

inline FloatType PurchasingManager::partial_D_r_scaled_D_r(const BusinessConnection* bc) const {
    return to_float(bc->initial_flow_Z_star().get_quantity());
}

inline FloatType PurchasingManager::scaled_objective(FloatType obj) const {
    return obj / partial_objective_scaled_objective();
}

inline FloatType PurchasingManager::unscaled_objective(FloatType x) const {
    return x * partial_objective_scaled_objective();
}

inline FloatType PurchasingManager::partial_objective_scaled_objective() const {
    return to_float(storage->initial_used_flow_U_star().get_quantity());
}

inline FloatType PurchasingManager::scaled_use(FloatType use) const {
    return use / partial_use_scaled_use();
}

inline FloatType PurchasingManager::unscaled_use(FloatType x) const {
    return x * partial_use_scaled_use();
}

inline FloatType PurchasingManager::partial_use_scaled_use() const {
    return to_float(storage->initial_used_flow_U_star().get_quantity());
}
...
FloatType
PurchasingManager::purchase_constraint(const FloatType x[], FloatType grad[], const OptimizerData* data) const {
#ifdef DEBUG
    try {
#endif
...
FloatType
PurchasingManager::objective_costs(const FloatType x[], FloatType grad[], const OptimizerData* data) const {
#ifdef DEBUG
    try {
#endif
        FloatType costs = 0.0;
...
FloatType PurchasingManager::expected_average_price_E_n_r(FloatType D_r,
...
FloatType PurchasingManager::grad_expected_average_price_E_n_r(FloatType D_r,
                                                               const BusinessConnection* business_connection) const {
    FloatType X = to_float(
            business_connection->seller->communicated_parameters().production_X.get_quantity());  // note: wo expectation X = X_expected;
    FloatType X_expected =
...
FloatType PurchasingManager::calc_n_co(FloatType n_bar_min,
                                       FloatType D_r_min,
                                       const BusinessConnection* business_connection) const {
    FloatType n_co =
...
FloatType
PurchasingManager::transport_penalty(FloatType D_r, const BusinessConnection* business_connection) const {
...
FloatType PurchasingManager::partial_D_r_transport_penalty(FloatType D_r,
                                                           const BusinessConnection* business_connection) const {
    FloatType target = 0.0;
    if (model()->parameters().deviation_penalty) {
        target = to_float(business_connection->last_demand_request_D(this).get_quantity());
...
void PurchasingManager::print_distribution(const FloatType demand_requests_D_p[],
