...
SalesManagerPrices::calc_expected_supply_distribution_scenario(const Flow& possible_production_X_hat_p) {
...
const FlowValue SalesManagerPrices::calc_total_production_costs(const Flow& production_X,
...
const FlowValue
SalesManagerPrices::calc_production_extension_penalty_P(const FlowQuantity& production_quantity_X) const {
    assert(production_quantity_X >= 0.0);
    if (production_quantity_X <=
        firm->forced_initial_production_quantity_lambda_X_star()) {  // not in production extension
...
const Price
SalesManagerPrices::calc_marginal_production_extension_penalty(const FlowQuantity& production_quantity_X) const {
    assert(production_quantity_X >= 0.0);
    if (production_quantity_X <=
        firm->forced_initial_production_quantity_lambda_X_star()) {  // not in production extension
        return Price(0.0);
...
const FlowValue SalesManagerPrices::calc_additional_revenue_expectation(const FlowQuantity& production_quantity_X_p,
...
const Price SalesManagerPrices::calc_marginal_revenue_curve(const FlowQuantity& production_quantity_X_p,
                                                            const Price& n_min_p) const {
    // note: for X > sum_of_demand_requests_D this curve corresponds to the marginal supply curve
    // note: communicated_parameters_.production_X == sum_demand_requests_D (up to rounding issues)
    assert(round(production_quantity_X_p) >= sum_demand_requests_D_.get_quantity());
...
const Price
SalesManagerPrices::goal_fkt_marginal_costs_minus_marginal_revenue(const FlowQuantity& production_quantity_X_p,
